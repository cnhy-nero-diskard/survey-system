...
export const login = async (req, res, next) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE admin_table SET last_login = NOW(), is_logged_in = TRUE WHERE username = $1',
      [username]
    );
...
export const logout = async (req, res, next) => {
  ...
  try {
    const result = await pool.query('SELECT last_login FROM admin_table WHERE username = $1', [
      username,
    ]);
    ...
    await pool.query(
      'UPDATE admin_table SET last_logout = NOW(), session_duration = $1, is_logged_in = FALSE WHERE username = $2',
      [sessionDuration, username]
    );
