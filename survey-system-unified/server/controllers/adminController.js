...
import {
  ...
  addTourismAttraction,
  fetchTourismAttraction,
  deleteTourismAttraction,
  fetchAnonymousUsers,
  getEstablishmentEnglishNames,
  fetchOpenEndedSurveyResponses,
  fetchSurveyQuestionsService,
...
import {
  ...
  getSentimentLocation,
  getSurveyResponseByTopic,
  groupByLikertRatingService,
...
export const getAdminSessionData = async (req, res, next) => {
  ...
  try {
    const result = await pool.query(
      'SELECT username, last_login, last_logout, session_duration, is_logged_in FROM admin_table'
    );
...
export const autoAnalyzeSentimentController = async (req, res) => {
  ...
  try {
    ...
    if (analysisResults.length !== relevantFeedback.rows.length) {
      throw new Error(
        `Mismatch in response count. Sent ${relevantFeedback.rows.length}, got ${analysisResults.length}`
      );
    }
...
export const autoClassifyRelevanceController = async (req, res) => {
  ...
  try {
    ...
    if (analysisResults.length !== unknownFeedback.rows.length) {
      throw new Error(
        `Mismatch in response count. Sent ${unknownFeedback.rows.length}, got ${analysisResults.length}`
      );
    }
    ...
    const updatePromises = unknownFeedback.rows.map((row, index) => {
      const result = analysisResults[index];
      return pool.query('UPDATE survey_feedback SET relevance = $1 WHERE response_id = $2', [
        result.relevance,
        row.response_id,
      ]);
    });
...
export const createSurveyResponseController = async (req, res, next) => {
  ...
  try {
    ...
    const result = await createSurveyResponseService(
      anonymous_user_id,
      surveyquestion_ref,
      response_value
    );
...
export const deleteSurveyResponseController = async (req, res, next) => {
  ...
  try {
    ...
    res.json({
      message: `Survey response ${anonymous_user_id} deleted successfully`,
      response: result,
    });
  } catch (err) {
...
export const updateSentimentAnalysisController = async (req, res, next) => {
  ...
  try {
    ...
    const result = await updateSentimentAnalysisService(
      id,
      user_id,
      review_date,
      rating,
...
export const insertTopicDataController = async (req, res, next) => {
  ...
  try {
    ...
    for (const item of [data]) {
      ...
      if (!top_words || !Array.isArray(top_words) || top_words.length === 0)
        missingFields.push('top_words');
...
export const getSurveyMetricsAnalyticsController = async (req, res, next) => {
  ...
  try {
    ...
    const dropOffRate = parseFloat(
      ((totalSurveysUnCompleted / totalMixedSurveys) * 100).toFixed(2)
    );
