...
const SurveyMetrics = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_API_HOST}/api/admin/getsurveymetrics`, {
  ...
  const surveyDistributionData = Object.entries(surveyMetrics.surveyDistribution).map(
    ([key, value]) => ({
      name: key,
      value,
    })
  ...
  const surveyResponsesByRegionData = Object.entries(surveyMetrics.surveyResponsesByRegion).map(
    ([key, value]) => ({
      name: key,
      value,
    })
  ...
  const surveyResponsesByCountryData = Object.entries(surveyMetrics.surveyResponsesByCountry).map(
    ([key, value]) => ({
      name: key,
      value,
    })
  ...
  const surveyResponsesByAgeGroupData = Object.entries(surveyMetrics.surveyResponsesByAgeGroup).map(
    ([key, value]) => ({
      name: key,
      value,
    })
  ...
  const surveyResponsesByMonthData = Object.entries(surveyMetrics.surveyResponsesByMonth).map(
    ([key, value]) => ({
  ...
  const exportToExcel = () => {
    ...
    const ageGroupSheet = createStyledSheet(
      surveyResponsesByAgeGroupData,
            ...
            >
              <Box flex={1}>
                ...
                <Box display="flex" alignItems="center" flexWrap="wrap" gap={1}>
                  ...
                  <StyledChip
                    label={`Period: ${capitalizeFirstLetter(getCurrentMonth())}`}
                    size="small"
                  />
                </Box>
              </Box>
          ...
          <Grid item xs={12} sm={6} lg={3}>
            <StyledMetricCard gradient="linear-gradient(135deg, #4ade80 0%, #22c55e 100%)">
              <CardContent>
                ...
                <Typography
                  variant="h4"
                  sx={{ color: theme.palette.success.main, fontWeight: 700, mb: 1 }}
                >
              ...
              <CardContent>
                ...
                {surveyMetrics.averageTimeToComplete != null ? (
                  <>
                    <Typography
                      variant="h4"
                      sx={{ color: theme.palette.warning.main, fontWeight: 700, mb: 1 }}
                    >
              ...
              <CardContent>
                ...
                {surveyMetrics.dropOffRate != null ? (
                  <>
                    <Typography
                      variant="h4"
                      sx={{ color: theme.palette.error.main, fontWeight: 700, mb: 1 }}
                    >
                    ...
                    {topThreeRegions.slice(0, 3).map((region, index) => (
                      <Box
                        ...
                        sx={{
                          mb: 1,
                          p: 1,
                          bgcolor: `rgba(6, 182, 212, ${0.1 + index * 0.05})`,
                          borderRadius: '8px',
                        }}
                      >
                        <Typography
                          variant="body1"
                          sx={{ fontWeight: index === 0 ? 700 : 500, color: '#1f2937' }}
                        >
                          {index + 1}. {region.name}
                        </Typography>
                    ...
                    {topThreeResidence.slice(0, 3).map((country, index) => (
                      <Box
                        ...
                        sx={{
                          mb: 1,
                          p: 1,
                          bgcolor: `rgba(236, 72, 153, ${0.1 + index * 0.05})`,
                          borderRadius: '8px',
                        }}
                      >
                        <Typography
                    ...
                    {topThreeAgeGroup.slice(0, 3).map((age, index) => (
                      <Box
                        ...
                        sx={{
                          mb: 1,
                          p: 1,
                          bgcolor: `rgba(132, 204, 22, ${0.1 + index * 0.05})`,
                          borderRadius: '8px',
                        }}
                      >
                        <Typography
                ...
                undefined ? (
                  <>
                    ...
                    <Typography variant="h4" sx={{ color: '#6366f1', fontWeight: 700, mb: 1 }}>
                      {
                        surveyMetrics.surveyResponsesByMonth[
                          capitalizeFirstLetter(getCurrentMonth())
                        ]
                      }
                    </Typography>
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {surveyDistributionData.map((row, index) => {
                            const total = surveyDistributionData.reduce(
                            ...
                            return (
                              <TableRow key={row.name}>
                                ...
                                <TableCell align="center">
                                  <Chip label={`${percentage}%`} color="secondary" size="small" />
                                </TableCell>
                              </TableRow>
                            );
                          })}
                  ...
                  <Box sx={{ p: 2, bgcolor: '#f8fafc', borderRadius: '15px' }}>
                    ...
                    <ResponsiveContainer width="100%" height={400}>
                      <BarChart data={sortedResidence.slice(0, 10)}>
                        ...
                        />
                        <Bar dataKey="value" fill="url(#countryGradient)" radius={[4, 4, 0, 0]} />
                        <defs>
