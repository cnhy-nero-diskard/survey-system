...
const LocSpecificTopic = ({ short_id }) => {
  ...
  useEffect(() => {
    ...
    const processData = async (counts = {}, positive = [], neutral = [], negative = []) => {
      ...
      if (positiveCount > 0) {
        data.push({
          name: positiveLabel ? `Positive (${positiveLabel})` : 'Positive',
          value: positiveCount,
      ...
      if (neutralCount > 0) {
        data.push({
          name: neutralLabel ? `Neutral (${neutralLabel})` : 'Neutral',
          value: neutralCount,
      ...
      if (negativeCount > 0) {
        data.push({
          name: negativeLabel ? `Negative (${negativeLabel})` : 'Negative',
          value: negativeCount,
    ...
    return (
      ...
      >
        <CircularProgress
          size={28}
          thickness={4}
          sx={{ color: sentimentPalette.positive }}
          aria-label="Loading topic sentiment"
        />
