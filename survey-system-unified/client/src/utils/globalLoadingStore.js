...
export const GlobalLoadingProvider = ({ children }) => {
  ...
  const setGlobalLoading = useCallback(
    (isLoading, message = 'Loading...', subtitle = '', showCircular = true) => {
  ...
  const setProcessingData = useCallback(
    (subtitle = 'Processing and analyzing survey responses') => {
      setGlobalLoading(true, 'Processing Data', subtitle, true);
    },
    [setGlobalLoading]
  );

  const setLoadingDashboard = useCallback(
    (dashboardType = 'Dashboard', subtitle = 'Loading analytics and insights') => {
  ...
  const getComponentLoadingMessage = useCallback(
    (componentId) => {
